...
namespace Logic.Services
{
    public class Administration : IAdministration
    {
        ...
        public async Task<Response<string>> CreateAsync(User _user)
        {
            ...
            var types = await userTyperepository.GetAll();
            var basicUser = types.FirstOrDefault();
            ...
            List<Claim> claims = new List<Claim>() { new Claim(basicUser.Name, basicUser.Descriprion) };

            if (_user.Type.Id == 2)
            {
                var admin = types.Where(t => t.Id == _user.Type.Id).First();
                claims.Add(new Claim(admin.Name, admin.Descriprion));
            }
